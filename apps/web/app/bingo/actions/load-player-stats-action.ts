...
import { bingoCompletions } from '@/lib/db/schema';
...
export async function loadPlayerStatsAction(): Promise<PlayerStatsResponse> {
    try {
        const [ironsGrottoStats, ironDaddyStats] = await Promise.all([
            getPlayerStatsForClan('ironsGrotto'),
            getPlayerStatsForClan('ironDaddy')
        ]);

        return {
            success: true,
            playerStats: {
                ironsGrotto: ironsGrottoStats,
                ironDaddy: ironDaddyStats
            }
        };
    } catch (error) {
...
async function getPlayerStatsForClan(team: 'ironsGrotto' | 'ironDaddy'): Promise<PlayerStats[]> {
    ...
    const userStats = {} as Record<string, PlayerStats>;
    ...
    Object.entries(taskGroups).forEach(([taskId, taskGroup]) => {
        ...
        Object.entries(userContributions).forEach(([user, contributionCount]) => {
            if (!userStats[user]) {
                userStats[user] = { user, completions: 0, points: 0, clan: team };
            }
    ...
    return Object.values(userStats)
        .map(user => ({
            ...user,
            points: Math.round(user.points * 100) / 100 // Round to 2 decimal places
        }))
        .sort((a, b) => {
