...
export function getAllItemNames(): string[] {
    if (cachedAllItemNames === null) {
        cachedAllItemNames = Array.from(itemNameToCategoryMap.keys());
    }
    return cachedAllItemNames;
}
 ...
 */
export function processTempleItems(acquiredItems: Record<string, boolean>): Array<{
    name: string;
    category: string;
    isValid: boolean;
}> {
