...
export async function createNewPlayer(data: CreatePlayerData): Promise<Player> {
    ...
    const [createdPlayer] = await db.insert(players).values(playerData).returning();
 ...
 */
export async function addNewItem(data: AddItemData): Promise<PlayerAcquiredItem> {
    const itemData: NewPlayerAcquiredItem = {
        ...data,
        dateFirstLogged: new Date(data.dateFirstLogged),
    };

    const [createdItem] = await db.insert(playerAcquiredItems).values(itemData).returning();
    return createdItem;
}
    ...
    filteredItems.forEach(item => {
        ...
        if (existing) {
            ...
            itemMap.set(item.itemId, {
                ...
                dateFirstLogged: existing.dateFirstLogged < item.dateFirstLogged ?
 ...
 */
export async function createOrUpdateAcquiredItem(data: AddItemData): Promise<PlayerAcquiredItem> {
...
export async function addAchievementDiary(
    ...
    const [createdDiary] = await db.insert(playerAchievementDiaries).values(data).returning();
    return createdDiary;
}
...
export async function createOrUpdateAchievementDiary(
    ...
    if (existingDiary) {
        ...
        const updated = await updateAchievementDiaryLevel(data.playerName, data.location, {
            tier: data.tier,
...
export async function getPlayerWithRelations(
    playerName: string
): Promise<(Player & { acquiredItems: PlayerAcquiredItem[]; achievementDiaries: PlayerAchievementDiary[] }) | null> {
 ...
 */
export async function syncPlayerToDatabase(playerData: PlayerDetailsResponse & {
    rank?: string;
    proofLink?: string;
}): Promise<void> {
    try {
        ...
        const postgresPlayerData: CreatePlayerData = {
            playerName,
            joinDate: joinDate instanceof Date ? joinDate.toISOString().split('T')[0] : joinDate,
        ...
        console.log(`Successfully synced player ${playerName} to postgres database`);
    } catch (error) {
