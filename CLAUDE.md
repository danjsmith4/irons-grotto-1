...
- **Two jobs, both authenticated with `CRON_SECRET`, both in `vercel.json`.** `GET /api/update-all-players` refreshes the stalest slice from Temple, hourly; `GET /api/reconcile-points` rescores the whole roster from the database, hourly. `isScheduledRequest` **fails closed** — with no `CRON_SECRET` set both return 401 and nothing runs, because an unset secret is indistinguishable from a broken deploy and the failure mode of guessing is a public endpoint hammering Temple. Vercel cron limits are plan-gated: hourly needs **Pro** (Hobby is capped at once per day and rejects a finer expression at deploy time), and `maxDuration` is 300s default on both.
