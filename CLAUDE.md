...
- **Nothing on the client writes points.** `useRank` / `useRankCalculator` / `useTotalPoints` are pure — the running total they produce is for display only, and `useTotalPoints` remains the cheaper option when just the number is needed. `players.points` is written only inside `processPlayerData`, by `scoreStoredPlayer` (`app/data-sources/score-players-from-record.ts`). Don't reintroduce a client-side write: the previous one posted to an **unauthenticated** action taking `(playerName, points)`, so the leaderboard was whatever the client last claimed, for whichever player it named. If scoring throws (it needs live wiki drop rates) the stored total is left alone — stale is a cosmetic lag, zero would be a catastrophe.

## Points are scored from the record, and only from the record

⚠️ **There is one scoring path and it reads Postgres.** `score-players-from-record.ts` is it; `build-points-breakdown.ts` is the arithmetic. `processPlayerData` scores **the row it just wrote**, not the `PlayerDetailsResponse` it wrote the row from — those are not the same thing, and the difference was a live bug.

There used to be a second path. `calculatePlayerPoints(PlayerDetailsResponse)` scored the live blend of Temple/WikiSync/wiki and froze the answer in `players.points`, while the comparison ledger scored the same player out of the database. Identical arithmetic over different inputs, so the two diverged whenever the record moved without a sync behind it — which is most of the time. **84 of 136 active members disagreed with their own breakdown, all of them scored low.** That file is deleted; don't reintroduce anything that scores a response.

Three consequences worth keeping:

- **Two jobs, both authenticated with `CRON_SECRET`.** `GET /api/update-all-players` refreshes the stalest slice from Temple; `GET /api/reconcile-points` rescores the whole roster from the database. `isScheduledRequest` **fails closed** — with no `CRON_SECRET` set both return 401 and nothing runs, because an unset secret is indistinguishable from a broken deploy and the failure mode of guessing is a public endpoint hammering Temple.
- ⚠️ **The schedule is `.github/workflows/refresh-players.yaml`, not `vercel.json`.** Production is Vercel **Hobby** (https://ironsgrotto.xyz), where a cron may run **at most once per day** and a finer expression is *rejected at deploy time* — so putting an hourly entry in `vercel.json` does not degrade, it **breaks the build**. `vercel.json` keeps daily entries as a backstop for when GitHub drops or delays a scheduled run; the workflow does the real work, hourly. It needs only the `CRON_SECRET` secret (the domain is a hardcoded default, overridable with a `SITE_URL` variable); `DISCORD_OPS_WEBHOOK` is optional and covers only total unreachability, which the endpoints cannot report for themselves.
- **Two schedulers means the jobs take a lease.** `claimJobLease` / `releaseJobLease` use the same atomic conditional upsert on `sync_metadata` that `maybeRunInactivitySync` does. It matters most for the refresh: it selects the players with the oldest `updated_at`, so an overlapping second run picks **the same** players the first has not finished and spends Temple's rate limit twice. The lease **expires** rather than being a flag, so a run killed mid-flight costs one window instead of wedging the job, and `claimJobLease` **fails open** — a duplicated run wastes rate limit, whereas refusing to run stops the data updating, which is the failure the whole subsystem exists to prevent.
- **Run size is `refreshBatchSize` (10), not the clock.** 10 an hour is 240 a day against 202 members, so everyone is refreshed daily with ~19% slack. `refreshTimeBudgetMs` is a backstop only: a healthy run ends because the batch ran out, and `stoppedForTime: true` is a signal that players were much slower than expected. Keeping runs short matters on Hobby, where 300s is the maximum as well as the default and cannot be raised.
- ⚠️ **The unscorable-record alert is rate limited to once a day** (`reconcileSkipAlertJobId`). The condition is persistent — a record stays unscorable until a person fixes it — so an hourly job would post the same names every hour, which trains the channel to ignore it. Don't remove the limiter to "make sure it's seen".
- ⚠️ **The third cron, `check-all-players`, does nothing.** It fans out to `/api/check-player` through **Zeplo**, and `serverConstants.zeplo` in `config/constants.server.ts` is hardcoded to empty strings — `ZEPLO_API_KEY` / `ZEPLO_URL` are in `.env.sample` but never read. So it has been POSTing to `/bulk?_token=` on an empty URL every Monday. Left in place rather than deleted because the Zeplo fan-out is still the right shape for the inactivity sweep, but **don't count it as a working schedule**; nothing it claims to do has happened.
- ⚠️ **The refresh is chunked because it cannot not be.** Temple's ~10 datapoint requests/minute means ~12s per player, so 200 members is ~40 minutes — longer than any function runs. A run takes the oldest `updated_at` first and stops on its own 240s budget inside the 300s ceiling, reporting `backlogRemaining` rather than dying at a 504. **Ordering by `updated_at` is what makes it self-healing**: a fixed 1/24 partition loses a whole bucket when its run fails, whereas a missed player is by definition the stalest and is first in line next hour. Don't replace it with a stored cursor.
- ⚠️ **`unscorableReason` (`app/utils/is-record-scorable.ts`) is a guard against scoring a half-written record.** A never-synced stub holds the schema defaults; a record with `collection_log_count > 0` and no `player_acquired_items` rows has lost the itemised half its notable items come from. Scoring either produces a number that looks like a score and is not one. Reconciliation leaves those totals alone and reports them. This matters because renames strand child rows — the child tables match on name, exactly and case-sensitively — and orphaned owners (`otsutsuki` beside `Otsutsuki`) already exist.
...
⚠️ **The collection-log match is by name, and `stripEntityName` only removes `'` and `.`** — so a casing change at Temple's end silently unmatches an item. Temple's names **do** drift: by September 2026 it had title-cased a swathe of them (`Ikkle Hydra`, `Pet Zilyana`, `Jar of Spirits`, `Remnant of Akkha`, `Tome of Fire (empty)`) and dropped the apostrophe from others (`Skull of Vetion`, `Lil Zik`), while `CollectionLogItemName` and every item config use the in-game casing.

`TempleOSRSCollectionLogItem` (`schemas/temple-api.ts`) therefore **re-cases Temple's `name` back to the canonical `CollectionLogItemName` at ingestion** — the exact twin of the `DroppedItemResponse` re-casing above, keyed on the *stripped, lowercased* name so it covers the apostrophe drift too. Guarded hermetically by `temple-api.spec.ts`. Because it normalises at the parse, `isItemAcquired`, the `player_acquired_items` write and the accomplishment name matches all stay stable; `bulkUpsertCollectionLogItems` refreshes `item_name` on conflict so a row written under an older spelling converges rather than sitting there unmatchable. Names still come from `collection-log/items.php`, not the wiki — the two drift apart independently (the wiki renamed `Mr mcgroot` → `Mr McGroot` in August 2026).
...
⚠️ **But one response is not the evidence.** `sourceValues.acquiredItems` is the live read **unioned with the stored collection log** — `buildSourceAcquiredItems` (pure, spec'd) is that rule. `player_acquired_items` is written only ever from Temple's own responses, so it is a source's past answer, not the member's word; only `player_item_overrides` (the member's own ticks) is held back, and flagging those is the diff's whole job. A collection log slot cannot be un-earned, so an item missing from a response means the response did not mention it — never that it is not owned. Comparing against a single response instead is what let the Temple casing drift above turn 23 of one member's own logged drops into fabricated "unverified claims" and block auto-approval on a real application; scoring never had the bug, because `buildPreviouslyAcquiredItems` already floors the live read with the same rows.

An earlier bug in the same place: extending `currentDbValues` to cover `hasBloodTorva` / `hasDizanasQuiver` / `hasAchievementDiaryCape` (the fix for a quiet source wiping them) also made the "fresh" side of the diff fall back to the stored claim, so an unverified claim stopped being flagged — and `approveSubmission` grants Discord roles off `!hasBloodTorvaDiscrepancy`. Don't collapse `sourceValues` back into the blended fields.
...
**After creating, the competition id is written to the clan's events sheet, then staff are told** (`announceClanEvent`). The sheet (`clanEventSheetId`) reads one cell per type — `clanEventSheetCell`: SOTW `Links!B10`, BOTW `Links!B9` — and the site makes that single `USER_ENTERED` write itself (`app/data-sources/google-sheets.ts`, a hand-signed service-account JWT; no `googleapis` dependency). It then posts to `#sotw-and-botw` (`clanEventAnnouncementChannelId`), pinging the Staff role, **whether or not the write landed** — on failure the message says so and hands staff the `.sotw <url>` / `.botw <url>` command that does the same write by hand. Both outcomes are reported, not rolled back (`handoff: { sheet, discord }`), matching the staff-role convention.

⚠️ **Do not go back to posting the bot command from the site.** That is what this replaced, and it never worked: Grotto Bot is a discord.py `commands.Bot`, whose `process_commands` ignores every message authored by a bot — so `.botw <url> <key>` landed in the channel and nothing ran it. A webhook is no way round it either (webhook authors are bots too). The sheet write needs `GOOGLE_SERVICE_ACCOUNT_KEY` (the JSON key, whole) for a service account with **edit** access to the sheet; without it the write fails and the message falls back to the manual command. The edit key is no longer sent to Discord — the sheet never needed it.
