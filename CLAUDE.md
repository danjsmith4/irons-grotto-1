...
Badges: `app/components/account-type-badge.tsx` renders the in-game chat badge from the wiki. Mains have no chat badge in game, so they fall back to the `mainAccountRank` (Looter) icon — **every resolved account gets a mark**, which is what keeps the leaderboard's names on one left edge. Only a null (unresolved) type renders nothing.
